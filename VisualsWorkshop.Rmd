...
And because the names of the regions are quite long and were causing the plots to be a bit clunky, we can rename them to shorter versions
...
What if we want the colours to be reflective of the data, for example darker for higher harvest?
...
plot18<- plot1 +
  geom_point(aes(colour=subs_lbs_per_capita_harvest), position=position_jitter(width=0.2))+
...
plot25<- plot1 +
  geom_point(aes(size=total_2016), colour="#0f3c52", alpha=0.5,
...
plot30<-plot28 + 
  labs(x="Latitude", y="Pounds per Capita", size="Total Population, 2016")
plot30
...
plot32<- ggplot(wellbeing.ak, aes(x=region, y=subs_lbs_per_capita_harvest)) +
  geom_bar(stat="summary", fun.y="mean") +
  geom_point(aes(colour=lat), position=position_jitter(0.2)) +
...
plot34<- plot1 +
  geom_point(aes(shape=region, colour=region, size=total_2016),
             position=position_jitter(width=0.2)) +
  scale_shape_manual(values=c(1:13)) +
