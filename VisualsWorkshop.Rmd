...
plot27<-ggplot(plotting.data, aes(x=median_income, y=var.y)) +
  geom_point(aes(colour=lat), position=position_jitter(width=0.2)) +
  scale_colour_continuous()+
  facet_wrap(vars(var.x), labeller=label_wrap_gen(width=20, multi_line=TRUE)) +
...
plotting.data$var.x[plotting.data$var.x=="Alaska Peninsula and Aleutian Islands"]<-"AP&AI"
plotting.data$var.x[plotting.data$var.x=="Cook Inlet"]<-"CI"
plotting.data$var.x[plotting.data$var.x=="Prince William Sound"]<-"PWS"
plotting.data$var.x[plotting.data$var.x=="Norton Sound"]<-"Norton S"
plotting.data$var.x[plotting.data$var.x=="Bristol Bay"]<-"Bristol B"
plotting.data$var.x[plotting.data$var.x=="Copper River"]<-"Copper R"

plot27b<-ggplot(plotting.data, aes(x=median_income, y=var.y)) +
  geom_point(aes(colour=lat), position=position_jitter(width=0.2)) +
  scale_colour_continuous()+
  facet_wrap(vars(var.x), labeller=label_wrap_gen(width=20, multi_line=TRUE)) +
  labs(title="Percent of Households\nUsing Subsistence Salmon\nby Median Income", subtitle="At the community level,\nacross SASAP regions", caption="Use of subsistence salmon from ADF&G surveys.\n Median income from US Census, 2016.", x="Median Income (USD)", y="Percent of Households", colour="Latitude") + 
...
plot16b<- ggplot(plotting.data, aes(x=var.x, y=var.y)) +
  geom_bar(aes(fill=var.x), stat="summary", fun.y="mean") +
  scale_colour_discrete()
plot16b
```
